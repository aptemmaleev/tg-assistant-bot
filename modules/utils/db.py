...
class PostgresDatabase:
    ...
    @classmethod
    def set_role(cls, user_id: int, role: str):
        """Set user's role
        ...
        Args:
            ...
            role (str): role name
        """
        PostgresDatabase.execute_query(f"update users set role='{role}' where id = {user_id}")
    ...
    @classmethod
    def add_group(cls, user_id: int, name: str, sheets_id: str):
        """Create new group and add it to user
        ...
        """
        PostgresDatabase.execute_query(f"""insert into groups ("user", "name", "sheets") values ({str(user_id)}, '{name}', '{sheets_id}')""")